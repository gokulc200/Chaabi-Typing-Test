...
import { quotesArray, random, allowedKeys } from './AssetData'
...
const App = () => {
	...
	const handleKeyDown = e => {
		...
		const _wpm = Math.round(correctIndex/ 3 / timeRemains) * (-1);
	...
	useEffect(() => {
		const previousScore = localStorage.getItem('wpm')
		if (previousScore) setLastScore(previousScore)
	}, [])
				...
				<div className="row">
						...
						<div className="container">
							...
							{ended ? (
								<div className="bg-dark text-light p-4 mt-5 lead rounded">
									...
									<span className="d-block mt-2 text-muted small">- {quote.letter}</span>
								</div>
							) : started ? (
								<div
					...
					<div className="col-sm-6 col-md-2 order-md-2 px-5">
						<ul className="list-unstyled text-center small">
							...
							<ItemList name="Acuracy" data={accuracy} percentage="%" />
						</ul>
					</div>
				</div>
			</div>
